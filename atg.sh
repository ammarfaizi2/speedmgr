...
if [ "$1" == "on" ]; then
	...
	sudo iptables -t nat -I OUTPUT -p tcp -m owner --uid-owner 1001 -j REDIRECT --to-ports 4444;
elif [ "$1" == "off" ]; then
	set -x;
	...
	sudo iptables -t nat -D OUTPUT -p tcp -m owner --uid-owner 1001 -j REDIRECT --to-ports 4444;
else
	echo "Usage: $0 {on|off}";
	exit 1;
fi
