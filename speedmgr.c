...
#define pr_error(fmt, ...) printf("perr: " fmt "\n", ##__VA_ARGS__)
...
struct server_wrk {
	...
	pthread_mutex_t		client_arr_mtx;
};
...
static int init_worker(struct server_wrk *w, bool create_thread)
{
	...
	ret = pthread_mutex_init(&w->client_arr_mtx, NULL);
	if (ret) {
		pr_error("Failed to initialize client array mutex: %s", strerror(ret));
		return -ret;
	}

	ret = init_clients(w);
	if (ret) {
		pthread_mutex_destroy(&w->client_arr_mtx);
		return ret;
	}
...
{
	struct client_state *c = NULL;
	uint32_t i;

	pthread_mutex_lock(&w->client_arr_mtx);
	for (i = 0; i < w->client_arr_size; i++) {
	...
	if (c) {
		assert(!c->is_used);
		ENDP_ASSERT(&c->client);
		ENDP_ASSERT(&c->target);
		assert(!c->target_connected);
		assert(!c->spd);
		assert(!c->rate_limit_flags);
		assert(!c->client_addr.sa.sa_family);
		assert(!c->target_addr.sa.sa_family);
		c->is_used = true;
		atomic_fetch_add(&w->nr_online_clients, 1u);
	}

	pthread_mutex_unlock(&w->client_arr_mtx);
	return c;
}
...
static void put_client_slot(struct server_wrk *w, struct client_state *c)
{
	...
	if (c->client.fd >= 0) {
		pr_infov("pcls: %s -> %s (fd=%d; thread=%u)", sockaddr_to_str(&c->client_addr), sockaddr_to_str(&c->target_addr), c->client.fd, w->idx);
			   ...
			   struct sockaddr_in46 *addr)
{
	...
	if (w->ctx->cfg.bind_addr.sa.sa_family == AF_INET6) {
		if (IN6_IS_ADDR_V4MAPPED(&c->client_addr.in6.sin6_addr)) {
			len = sizeof(addr->in4);
				      ...
				      struct sockaddr_in46 *addr)
{
	...
	pr_infov("accp: %s -> %s", sockaddr_to_str(&c->client_addr), sockaddr_to_str(&c->target_addr));
	atomic_fetch_add(&w->nr_online_clients, 1u);
	return 0;
}
...
static int handle_event_target_conn(struct server_wrk *w, struct epoll_event *ev)
{
	...
	if (unlikely(tmp || !(events & EPOLLOUT))) {
		if (tmp < 0)
			tmp = -tmp;
		pr_error("Connect error: %s: %s", sockaddr_to_str(&c->target_addr), strerror(tmp));
		return -ECONNRESET;
	}
	...
	data.ptr = c;
	data.u64 |= EPL_EV_TCP_TARGET_DATA;
	c->target.ep_mask = EPOLLIN | (c->client.len > 0 ? EPOLLOUT : 0);
	...
	pr_infov("conn: %s -> %s", sockaddr_to_str(&c->client_addr), sockaddr_to_str(&c->target_addr));
	return 0;
}
			  ...
			  size_t max_send)
{
	...
	if (ret == 0) {
		printf("do_ep_send: Connection reset by peer\n");
		return -ECONNRESET;
	}
...
{
	struct sockaddr_in46 *psrc = (src == &c->client) ? &c->client_addr : &c->target_addr;
	struct sockaddr_in46 *pdst = (dst == &c->client) ? &c->client_addr : &c->target_addr;
	__unused const char *src_name = (src == &c->client) ? "Client" : "Target";
	__unused const char *dst_name = (dst == &c->client) ? "Client" : "Target";
	__unused enum tkn_type type = (src == &c->client) ? UP_TKN : DN_TKN;
	size_t max_send = MAX_RECV_BUF_RESIZE;
	ssize_t ret;
	...
	if (ret < 0) {
		...
		if (ret == -ENOBUFS) {
			pr_debug("%s recv buffer is full, disabling EPOLLIN (thread %u)", src_name, w->idx);
	...
	if (ret < 0) {
		if (ret != -EAGAIN) {
			pr_error("pi: send() to %s (psrc: %s; pdst: %s): %s", dst_name, sockaddr_to_str(psrc), sockaddr_to_str(pdst), strerror(-ret));
			return ret;
		}

		pr_debug("%s send buffer is full, enabling EPOLLOUT (thread %u)", dst_name, w->idx);
			     ...
			     struct client_endp *src, struct client_endp *dst)
{
	...
	if (src->len == 0) {
		pr_debug("%s send buffer has been flushed, disabling EPOLLOUT (thread %u)", src_name, w->idx);
	...
	if ((src->cap - src->len) > 0 && !(src->ep_mask & EPOLLIN)) {
		pr_debug("%s recv buffer has space, enabling EPOLLIN (thread %u)", src_name, w->idx);
...
static int handle_event_target_data(struct server_wrk *w, struct epoll_event *ev)
{
	...
	if (unlikely(events & (EPOLLERR | EPOLLHUP))) {
		pr_errorv("rhup: Target socket hit (EPOLLERR|EPOLLHUP): %s -> %s (thread %u)", sockaddr_to_str(&c->client_addr), sockaddr_to_str(&c->target_addr), w->idx);
		return -ECONNRESET;
	}
